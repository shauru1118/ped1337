...
class UserSessionManager:
    ...
    def set_pending_payload(self, user_id: int, payload_path: Path, mode: str = None) -> None:
        session = self.get_session(user_id)
        session.pending_payload_path = payload_path
        session.pending_payload_mode = mode

    def reset(self, user_id: int) -> Tuple[Optional[Path], Optional[Path], Optional[Path]]:
...
class StegoBotService:
    ...
    def __init__(self):
        ...
        self.key_file_path = str(Path(__file__).parent.resolve() / "key.key")
